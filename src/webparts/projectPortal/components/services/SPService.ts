...
const LIST_API_FASER_ENDPOINT: string = `/_api/web/lists/GetByTitle('ProjektTyp')/items`;
...
export const fetchFaser = async (absoluteSiteUrl: string):Promise<any> =>{
  const selectQuery = '?$select=Title,Id';
  const query = `${absoluteSiteUrl}${LIST_API_FASER_ENDPOINT}${selectQuery}`;
  try{
         const rawResponse = await fetch(query, {
             headers: {
                 Accept: 'application/json',
